...
def generate_pdf_report(df, dataset_name):
    ...
    story.append(
        Paragraph(
            f"<b>Total Columns:</b> {len(df.columns):,}",
            normal_style
        )
    )
    ...
    if numeric_df.empty:
        ...
        for column in statistics.index:
            ...
            statistics_data.append([
                str(column),
                f"{statistics.loc[column, 'count']:.2f}",
                f"{statistics.loc[column, 'mean']:.2f}",
                f"{statistics.loc[column, 'std']:.2f}",
                f"{statistics.loc[column, 'min']:.2f}",
                f"{statistics.loc[column, 'max']:.2f}"
            ])
    ...
    story.append(
        Paragraph(
            "Generated by Retail Detective AI",
            normal_style
        )
    )
...
def show_dashboard():
    ...
    # ========================================================
        ...
        if uploaded_file is not None:
            ...
            try:
                ...
                if not user_id:
                    st.error("❌ User session is missing. Please log in again.")
                    st.stop()

                upload_signature = (
                    f"{user_id}:{uploaded_file.name}:{uploaded_file.size}"
                )

                # Streamlit reruns frequently, so save this upload only once.
                if st.session_state.get("dataset_upload_signature") != upload_signature:
                    ...
                    if dataset_id is None:
                        st.error("❌ Dataset metadata could not be saved to PostgreSQL.")
                        st.stop()

                    raw_saved = save_dataset_rows(
                    ...
                    if not raw_saved:
                        ...
                        st.error("❌ Dataset rows could not be saved to PostgreSQL.")
                        st.stop()
                ...
                st.success(
                    "✅ Dataset uploaded successfully and stored in PostgreSQL!"
                )
                st.caption(f"PostgreSQL Dataset ID: {dataset_id}")
    ...
    # ========================================================
        ...
        if "dataset" not in st.session_state:
            ...
            if st.button(
                ...
                missing_values_before = int(cleaned_df.isnull().sum().sum())
                ...
                # ------------------------------------------------
                # SAVE CLEANED DATA + CLEANING HISTORY
                # ------------------------------------------------
                dataset_id = st.session_state.get("dataset_id")

                if not dataset_id:
                    st.error("❌ Dataset ID is missing. Please upload the dataset again.")
                    st.stop()
                ...
                delete_dataset_version(dataset_id, "cleaned")

                cleaned_saved = save_dataset_rows(
                    dataset_id=dataset_id,
                    dataframe=cleaned_df,
                    data_version="cleaned"
                )

                if not cleaned_saved:
                    st.error("❌ Cleaned data could not be saved to PostgreSQL.")
                    st.stop()

                log_saved = create_cleaning_log(
                    dataset_id=dataset_id,
                ...
                if not log_saved:
                    st.warning("⚠️ Cleaned data was saved, but the cleaning log could not be recorded.")

                update_dataset_status(dataset_id, "cleaned")
                st.session_state.cleaned_dataset = cleaned_df.copy()

                st.success(
                    "✅ Cleaning applied and cleaned data stored in PostgreSQL!"
                )
        ...
        if "dataset" not in st.session_state:
            ...
            try:
                ...
                question = st.text_input(
                    "Type your question",
                    placeholder=(
                        "Example: What is the minimum Amount?"
                    ),
                    key="ai_question"
                )
                ...
                if st.button(
                    ...
                    if not question.strip():
                        ...
                        st.session_state.ai_show_column_selector = (
                            True
                        )
                ...
                if st.session_state.get(
                    ...
                    if (
                        detected_column is not None
                        or not column_required
                    ):
    ...
    # ========================================================
        ...
        st.download_button(
            ...
            file_name="Retail_Detective_Report.xlsx",
            mime=(
        ...
        try:
            ...
            st.download_button(
                ...
                data=pdf_data,
                file_name="Retail_Detective_Report.pdf",
